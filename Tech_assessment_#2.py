...
# MySQL database configuration – update these details with your own credentials.
DB_CONFIG = {
    "host": "localhost",
    "user": "root",
    "password": "010821",
...
def filter_forecast_data(forecast_data, start_date, end_date):
    """
    Filter forecast entries so that only entries with dates within
    the provided date range are returned.
    """
    filtered = []
...
def validate_date_range(start_date_str, end_date_str):
    """
    Validate date strings (format: YYYY-MM-DD) and ensure the start date
